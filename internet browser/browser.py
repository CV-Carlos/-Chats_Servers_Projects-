...
def main():
  ...
  entry_page.bind('<Return>', lambda o:callback(pageList))
...
def createWidgets(pageList, thereIsLinks):
  ...
  page2.grid(row=len(pageList) + 3, column=0, columnspan=1, sticky='W')
  if thereIsLinks == False:
    page2.grid_forget()

def callback(pageList):
  if entry_page.get() != '':
...
def printMethod(data, pageList):
  ...
  linetrack = []
  for token in tokens:    
    token = token.decode('utf8') #Used to eliminate b' by changing the format
    print (token)
    if token == '<a':
      startPrint = False
      startCopyLink = True
